// docker run --gpus all -v /raid/dldata:/data:ro -v /run/udev:/run/udev:ro  --shm-size=1G --ulimit memlock=-1 --ulimit stack=67108864 --ulimit core=-1 -it -v /mnt/nvdl/usr/klecki:/klecki  urm.nvidia.com/sw-dl-dali-docker-local/dali-ci/main/cu102/x86_64/devel/py3.8:2511553 bash

// /bin/bash -c 'nvidia-smi || echo '\''nvidia-smi error ignored'\'' && cd /opt/dali/qa/TL0_tensorflow_plugin && exec bash -ex ./test.sh'
 ...
 private:
  ...
  Status GetExampleFromInput(IteratorContext *context, int input_idx, Tensor &example,
    ...
    *end_of_sequence = false;
    ...
    std::cout << "<< Obtained example <" << example.shape() << std::endl;
    return Status::OK();
  }
  ...
  Status PrepareBatchesBatchMode(IteratorContext *context, int input_idx, Batch &out_batch,
    ...
    *end_of_sequence = false;
    ...
    TF_RETURN_IF_ERROR(GetExampleFromInput(context, input_idx, in_batch[0], end_of_sequence));
    if (*end_of_sequence) {
      return Status::OK();
    }
...
// #endif
  ...
  Status RepackBatch(std::vector<const void *> &ptrs, dali_data_type_t &dtype,
    ...
    if (batched) {
      ...
      printf("Batched Sample : %d vs %d\n", batch_size, tensor.dim_size(0));
