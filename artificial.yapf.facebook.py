...
def get_required_kwargs(fun, skip_positional=0):
    ...
    params = [
        (name, param) for i, (name, param) in enumerate(sig.parameters.items())
...
def DALIIteratorWrapper(
    pipeline=None, serialized_pipeline=None, sparse=[], shapes=[], dtypes=[], batch_size=-1,
    prefetch_queue_depth=2, **kwargs
):
    """
  TF Plugin Wrapper
