 ...
 *********************************/
typedef CUresult  tcuDriverGetVersion(int *driverVersion);

/*********************************
 ** Error management
 *********************************/

typedef CUresult tcuGetErrorString(CUresult error, const char **pStr);
...
typedef CUresult  tcuDeviceComputeCapability(int *major, int *minor, CUdevice dev);
#if __CUDA_API_VERSION >= 3020
typedef CUresult  tcuDeviceTotalMem(size_t *bytes, CUdevice dev);
#else
typedef CUresult  tcuDeviceTotalMem(unsigned int *bytes, CUdevice dev);
#endif

typedef CUresult  tcuDeviceGetProperties(CUdevprop *prop, CUdevice dev);
typedef CUresult  tcuDeviceGetAttribute(int *pi, CUdevice_attribute attrib, CUdevice dev);
...
typedef CUresult  tcuCtxCreate(CUcontext *pctx, unsigned int flags, CUdevice dev);
typedef CUresult  tcuCtxDestroy(CUcontext ctx);
typedef CUresult  tcuCtxAttach(CUcontext *pctx, unsigned int flags);
typedef CUresult  tcuCtxDetach(CUcontext ctx);
typedef CUresult  tcuCtxPushCurrent(CUcontext ctx);
typedef CUresult  tcuCtxPopCurrent(CUcontext *pctx);

typedef CUresult  tcuCtxSetCurrent(CUcontext ctx);
typedef CUresult  tcuCtxGetCurrent(CUcontext *pctx);

typedef CUresult  tcuCtxGetDevice(CUdevice *device);
typedef CUresult  tcuCtxSynchronize(void);


typedef CUresult tcuDevicePrimaryCtxRetain(CUcontext *pctx, CUdevice dev);
typedef CUresult tcuDevicePrimaryCtxRelease(CUdevice dev);
...
typedef CUresult  tcuModuleLoadFatBinary(CUmodule *module, const void *fatCubin);
typedef CUresult  tcuModuleUnload(CUmodule hmod);
typedef CUresult  tcuModuleGetFunction(CUfunction *hfunc, CUmodule hmod, const char *name);

#if __CUDA_API_VERSION >= 3020
typedef CUresult  tcuModuleGetGlobal(CUdeviceptr *dptr, size_t *bytes, CUmodule hmod, const char *name);
#else
typedef CUresult  tcuModuleGetGlobal(CUdeviceptr *dptr, unsigned int *bytes, CUmodule hmod, const char *name);
#endif
...
typedef CUresult tcuMemGetAddressRange(CUdeviceptr *pbase, size_t *psize, CUdeviceptr dptr);
typedef CUresult tcuMemAllocPitch(CUdeviceptr *dptr,
                                          size_t *pPitch,
                                          size_t WidthInBytes,
                                          size_t Height,
...
#else
typedef CUresult tcuMemGetInfo(unsigned int *free, unsigned int *total);
typedef CUresult tcuMemAlloc(CUdeviceptr *dptr, unsigned int bytesize);
typedef CUresult tcuMemGetAddressRange(CUdeviceptr *pbase, unsigned int *psize, CUdeviceptr dptr);
typedef CUresult tcuMemAllocPitch(CUdeviceptr *dptr,
                                          unsigned int *pPitch,
...
#if __CUDA_API_VERSION >= 3020
typedef CUresult tcuMemAllocHost(void **pp, size_t bytesize);
typedef CUresult tcuMemHostGetDevicePointer(CUdeviceptr *pdptr, void *p, unsigned int Flags);
#else
typedef CUresult tcuMemAllocHost(void **pp, unsigned int bytesize);
#endif

typedef CUresult tcuMemFreeHost(void *p);
typedef CUresult tcuMemHostAlloc(void **pp, size_t bytesize, unsigned int Flags);

typedef CUresult tcuMemHostGetFlags(unsigned int *pFlags, void *p);
...
typedef CUresult tcuDeviceGetByPCIBusId(CUdevice *dev, char *pciBusId);
typedef CUresult tcuDeviceGetPCIBusId(char *pciBusId, int len, CUdevice dev);
typedef CUresult tcuIpcGetEventHandle(CUipcEventHandle *pHandle, CUevent event);
typedef CUresult tcuIpcOpenEventHandle(CUevent *phEvent, CUipcEventHandle handle);
typedef CUresult tcuIpcGetMemHandle(CUipcMemHandle *pHandle, CUdeviceptr dptr);
typedef CUresult tcuIpcOpenMemHandle(CUdeviceptr *pdptr, CUipcMemHandle handle, unsigned int Flags);
typedef CUresult tcuIpcCloseMemHandle(CUdeviceptr dptr);
#endif

typedef CUresult tcuMemHostRegister(void *p, size_t bytesize, unsigned int Flags);
typedef CUresult tcuMemHostUnregister(void *p);;
typedef CUresult tcuMemcpy(CUdeviceptr dst, CUdeviceptr src, size_t ByteCount);
typedef CUresult tcuMemcpyPeer(CUdeviceptr dstDevice, CUcontext dstContext, CUdeviceptr srcDevice, CUcontext srcContext, size_t ByteCount);
...
typedef CUresult  tcuMemcpyDtoD(CUdeviceptr dstDevice, CUdeviceptr srcDevice, size_t ByteCount);

// device <-> array memory
typedef CUresult  tcuMemcpyDtoA(CUarray dstArray, size_t dstOffset, CUdeviceptr srcDevice, size_t ByteCount);
typedef CUresult  tcuMemcpyAtoD(CUdeviceptr dstDevice, CUarray srcArray, size_t srcOffset, size_t ByteCount);

// system <-> array memory
typedef CUresult  tcuMemcpyHtoA(CUarray dstArray, size_t dstOffset, const void *srcHost, size_t ByteCount);
typedef CUresult  tcuMemcpyAtoH(void *dstHost, CUarray srcArray, size_t srcOffset, size_t ByteCount);
...
#if __CUDA_API_VERSION >= 3020
// system <-> device memory
typedef CUresult  tcuMemcpyHtoDAsync(CUdeviceptr dstDevice,
                                             const void *srcHost, size_t ByteCount, CUstream hStream);
typedef CUresult  tcuMemcpyDtoHAsync(void *dstHost,
                                             CUdeviceptr srcDevice, size_t ByteCount, CUstream hStream);

// device <-> device memory
typedef CUresult  tcuMemcpyDtoDAsync(CUdeviceptr dstDevice,
                                             CUdeviceptr srcDevice, size_t ByteCount, CUstream hStream);

// system <-> array memory
typedef CUresult  tcuMemcpyHtoAAsync(CUarray dstArray, size_t dstOffset,
                                             const void *srcHost, size_t ByteCount, CUstream hStream);
typedef CUresult  tcuMemcpyAtoHAsync(void *dstHost, CUarray srcArray, size_t srcOffset,
...
// system <-> array memory
typedef CUresult  tcuMemcpyHtoAAsync(CUarray dstArray, unsigned int dstOffset,
                                             const void *srcHost, unsigned int ByteCount, CUstream hStream);
typedef CUresult  tcuMemcpyAtoHAsync(void *dstHost, CUarray srcArray, unsigned int srcOffset,
                                             unsigned int ByteCount, CUstream hStream);
#endif

// 2D memcpy
typedef CUresult  tcuMemcpy2DAsync(const CUDA_MEMCPY2D *pCopy, CUstream hStream);
...
#if __CUDA_API_VERSION >= 3020
typedef CUresult  tcuMemsetD2D8(CUdeviceptr dstDevice, unsigned int dstPitch, unsigned char uc, size_t Width, size_t Height);
typedef CUresult  tcuMemsetD2D16(CUdeviceptr dstDevice, unsigned int dstPitch, unsigned short us, size_t Width, size_t Height);
typedef CUresult  tcuMemsetD2D32(CUdeviceptr dstDevice, unsigned int dstPitch, unsigned int ui, size_t Width, size_t Height);
#else
typedef CUresult  tcuMemsetD2D8(CUdeviceptr dstDevice, unsigned int dstPitch, unsigned char uc, unsigned int Width, unsigned int Height);
...
typedef CUresult tcuFuncGetAttribute(int *pi, CUfunction_attribute attrib, CUfunction hfunc);
typedef CUresult tcuFuncSetCacheConfig(CUfunction hfunc, CUfunc_cache config);
typedef CUresult tcuFuncSetSharedMemConfig(CUfunction hfunc, CUsharedconfig config);

typedef CUresult tcuLaunchKernel(CUfunction f,
...
typedef CUresult  tcuArray3DCreate(CUarray *pHandle, const CUDA_ARRAY3D_DESCRIPTOR *pAllocateArray);
typedef CUresult  tcuArray3DGetDescriptor(CUDA_ARRAY3D_DESCRIPTOR *pArrayDescriptor, CUarray hArray);

#if __CUDA_API_VERSION >= 5000
typedef CUresult tcuMipmappedArrayCreate(CUmipmappedArray *pHandle, const CUDA_ARRAY3D_DESCRIPTOR *pMipmappedArrayDesc, unsigned int numMipmapLevels);
typedef CUresult tcuMipmappedArrayGetLevel(CUarray *pLevelArray, CUmipmappedArray hMipmappedArray, unsigned int level);
...
typedef CUresult  tcuTexRefSetArray(CUtexref hTexRef, CUarray hArray, unsigned int Flags);

#if __CUDA_API_VERSION >= 3020
typedef CUresult  tcuTexRefSetAddress(size_t *ByteOffset, CUtexref hTexRef, CUdeviceptr dptr, size_t bytes);
typedef CUresult  tcuTexRefSetAddress2D(CUtexref hTexRef, const CUDA_ARRAY_DESCRIPTOR *desc, CUdeviceptr dptr, size_t Pitch);
#else
typedef CUresult  tcuTexRefSetAddress(unsigned int *ByteOffset, CUtexref hTexRef, CUdeviceptr dptr, unsigned int bytes);
typedef CUresult  tcuTexRefSetAddress2D(CUtexref hTexRef, const CUDA_ARRAY_DESCRIPTOR *desc, CUdeviceptr dptr, unsigned int Pitch);
#endif

typedef CUresult  tcuTexRefSetFormat(CUtexref hTexRef, CUarray_format fmt, int NumPackedComponents);
typedef CUresult  tcuTexRefSetAddressMode(CUtexref hTexRef, int dim, CUaddress_mode am);
typedef CUresult  tcuTexRefSetFilterMode(CUtexref hTexRef, CUfilter_mode fm);
typedef CUresult  tcuTexRefSetFlags(CUtexref hTexRef, unsigned int Flags);

typedef CUresult  tcuTexRefGetAddress(CUdeviceptr *pdptr, CUtexref hTexRef);
typedef CUresult  tcuTexRefGetArray(CUarray *phArray, CUtexref hTexRef);
typedef CUresult  tcuTexRefGetAddressMode(CUaddress_mode *pam, CUtexref hTexRef, int dim);
 ...
 **
 ***********************************/
typedef CUresult tcuEventCreate(CUevent *phEvent, unsigned int Flags);
typedef CUresult tcuEventRecord(CUevent hEvent, CUstream hStream);
typedef CUresult tcuEventQuery(CUevent hEvent);
typedef CUresult tcuEventSynchronize(CUevent hEvent);
typedef CUresult tcuEventDestroy(CUevent hEvent);
typedef CUresult tcuEventElapsedTime(float *pMilliseconds, CUevent hStart, CUevent hEnd);
...
#if __CUDA_API_VERSION >= 3020
typedef CUresult tcuGraphicsResourceGetMappedPointer(CUdeviceptr *pDevPtr, size_t *pSize, CUgraphicsResource resource);
#else
typedef CUresult tcuGraphicsResourceGetMappedPointer(CUdeviceptr *pDevPtr, unsigned int *pSize, CUgraphicsResource resource);
#endif
 ...
 **
 ***********************************/
typedef CUresult tcuGetExportTable(const void **ppExportTable, const CUuuid *pExportTableId);
...
typedef CUresult tcuCtxSetLimit(CUlimit limit, size_t value);
typedef CUresult tcuCtxGetLimit(size_t *pvalue, CUlimit limit);
typedef CUresult tcuCtxGetCacheConfig(CUfunc_cache *pconfig);
typedef CUresult tcuCtxSetCacheConfig(CUfunc_cache config);
typedef CUresult tcuCtxGetSharedMemConfig(CUsharedconfig *pConfig);
typedef CUresult tcuCtxSetSharedMemConfig(CUsharedconfig config);
 ...
 **
 ***********************************/
typedef CUresult tcuProfilerStop(void);

/************************************
 ************************************/

typedef CUresult tcuPointerGetAttribute(void* data, CUpointer_attribute attribute, CUdeviceptr ptr);

extern CUresult cuInit(unsigned int, int cudaVersion);
