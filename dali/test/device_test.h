    ...
    if (!(a_result op b_result)) {                                                                \
      ...
      if (nmsg < MAX_DEVICE_ERROR_MESSAGES && !test_status.fatal)                                 \
        printf("Check failed at %s:%i:\n%s ( = %s) " #op " %s ( = %s)\n", __FILE__, __LINE__, #a, \
               a_str.c_str(), #b, b_str.c_str());                                                 \
      test_status.failed = true;                                                                  \
      if (is_fatal)                                                                               \
        test_status.fatal = true;                                                                 \
    }                                                                                             \
  }
...
#define DEV_EXPECT_EQ(a, b) DEV_CHECK_OP_AB(a, b, ==, false)

#define DEV_EXPECT_NE(a, b) DEV_CHECK_OP_AB(a, b, !=, false)

#define DEV_EXPECT_LT(a, b) DEV_CHECK_OP_AB(a, b, <, false)

#define DEV_EXPECT_GT(a, b) DEV_CHECK_OP_AB(a, b, >, false)

#define DEV_EXPECT_LE(a, b) DEV_CHECK_OP_AB(a, b, <=, false)

#define DEV_EXPECT_GE(a, b) DEV_CHECK_OP_AB(a, b, >=, false)

#define DEV_EXPECT_TRUE(cond) DEV_CHECK_TRUE(cond, false)

#define DEV_EXPECT_FALSE(cond) DEV_CHECK_FALSE(cond, false)

#define DEV_ASSERT_EQ(a, b) DEV_CHECK_OP_AB(a, b, ==, true)

#define DEV_ASSERT_NE(a, b) DEV_CHECK_OP_AB(a, b, !=, true)

#define DEV_ASSERT_LT(a, b) DEV_CHECK_OP_AB(a, b, <, true)

#define DEV_ASSERT_GT(a, b) DEV_CHECK_OP_AB(a, b, >, true)

#define DEV_ASSERT_LE(a, b) DEV_CHECK_OP_AB(a, b, <=, true)

#define DEV_ASSERT_GE(a, b) DEV_CHECK_OP_AB(a, b, >=, true)

#define DEV_ASSERT_TRUE(cond) DEV_CHECK_TRUE(cond, true)

#define DEV_ASSERT_FALSE(cond) DEV_CHECK_FALSE(cond, true)
...
#define DECLARE_TEST_KERNEL(suite_name, test_name)                                          \
  ...
  __global__ void suite_name##_##test_name##_kernel(dali::testing::TestStatus *test_status, \
                                                    Args... args)

#define DEFINE_TEST_KERNEL(suite_name, test_name, ...)                                      \
  __device__ void suite_name##_##test_name##_body(dali::testing::TestStatus &test_status,   \
                                                  ##__VA_ARGS__);                           \
                                                                                            \
  template <typename... Args>                                                               \
  __global__ void suite_name##_##test_name##_kernel(dali::testing::TestStatus *test_status, \
