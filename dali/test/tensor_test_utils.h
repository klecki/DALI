...
bool IsEqWithConvert(Output out, Reference ref, int ulp = 4) {
  for (int i = 0; i <= ulp; i++) {
    if (dali::ConvertSat<Output>(ref) == out) return true;
    ...
    if (s1.tensor_shape_span(i) != s2.tensor_shape_span(i)) {
      if (++errors < max_errors) {
        EXPECT_EQ(s1.tensor_shape_span(i), s2.tensor_shape_span(i))
            << "Samples #" << i << " have different shapes " << s1[i] << " vs " << s2[i];
      }
    }
  }
...
inline int printable(char c) {
  return c;
}

inline int printable(signed char c) {
  return c;
}

inline int printable(unsigned char c) {
  return c;
}

template <typename StorageBackend, typename T1, typename T2, int dim1, int dim2,
          typename ElementsOkFunc>
void Check(const TensorView<StorageBackend, T1, dim1> &tv1,
  ...
  ASSERT_EQ(tv1.shape, tv2.shape) << "Tensors have different shapes " << tv1.shape << " vs "
                                  << tv2.shape << "\n";
...
if_iterable<Collection, void> Fill(Collection &&collection, Generator &&generator) {
  for (auto &x : collection) x = generator();
}
...
template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, std::uniform_real_distribution<T>>
uniform_distribution(T lo, T hi) {
  return std::uniform_real_distribution<T>(lo, hi);
}

template <typename T>
std::enable_if_t<std::is_integral<T>::value, std::uniform_int_distribution<T>> uniform_distribution(
    T lo, T hi) {
  return std::uniform_int_distribution<T>(lo, hi);
}

template <typename Collection, typename RandomGenerator>
if_iterable<Collection, void> UniformRandomFill(Collection &&c, RandomGenerator &rng,
                                                element_t<Collection> lo,
...
void UniformRandomFill(const TensorView<StorageBackend, DataType, ndim> &tv,
...
void UniformRandomFill(const TensorListView<StorageBackend, DataType, ndim> &tlv,
  ...
  static_assert(is_cpu_accessible<StorageBackend>::value,
                "Function available only for CPU-accessible TensorListView backend");
  for (int i = 0; i < tlv.num_samples(); i++) UniformRandomFill(tlv[i], generator, lo, hi);
}

template <typename C>
if_iterable<C, void> ConstantFill(C &&c, const element_t<C> &value = {}) {
  for (auto &x : c) x = value;
}

template <typename StorageBackend, typename DataType, int dim>
void ConstantFill(const TensorView<StorageBackend, DataType, dim> &tv,
...
void ConstantFill(const TensorListView<StorageBackend, DataType, dim> &tlv,
  ...
  static_assert(is_cpu_accessible<StorageBackend>::value,
                "Function available only for CPU-accessible TensorListView backend");
  for (int i = 0; i < tlv.num_samples(); i++) ConstantFill(tlv[i], value);
}
...
template <typename StorageBackend, typename DataType, int dim>
void SequentialFill(const TensorView<StorageBackend, DataType, dim> &tv,
...
void SequentialFill(const TensorListView<StorageBackend, DataType, dim> &tlv,
  ...
  static_assert(is_cpu_accessible<StorageBackend>::value,
                "Function available only for CPU-accessible TensorListView backend");
  for (int i = 0; i < tlv.num_samples(); i++) SequentialFill(tlv[i], start_value);
}
...
std::string BatchToStr(const TensorListView &batch, const std::string &sample_prefix = "Sample ") {
  ...
  for (int i = 0; i < batch.num_samples(); i++) {
    ss << sample_prefix << i << ":";
    for (auto &x : make_span(batch[i].data, batch[i].num_elements())) ss << " " << x;
  }
  return ss.str();
}
