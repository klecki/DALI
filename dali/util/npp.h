...
inline static NppStatus nppiResize_8u_C1R(const Npp8u *pSrc, int nSrcStep, NppiSize oSrcSize,
                                          NppiRect oSrcRectROI, Npp8u *pDst, int nDstStep,
                                          NppiSize oDstSize, NppiRect oDstRectROI,
                                          int eInterpolation) {
  if (oDstRectROI.width == oDstSize.width && oDstRectROI.height == oDstSize.height &&
      oDstRectROI.x == 0 && oDstRectROI.y == 0)
    return nppiResize_8u_C1R(pSrc, oSrcSize, nSrcStep, oSrcRectROI, pDst, nDstStep, oDstSize,
...
inline static NppStatus nppiResize_8u_C3R(const Npp8u *pSrc, int nSrcStep, NppiSize oSrcSize,
                                          NppiRect oSrcRectROI, Npp8u *pDst, int nDstStep,
                                          NppiSize oDstSize, NppiRect oDstRectROI,
                                          int eInterpolation) {
  if (oDstRectROI.width == oDstSize.width && oDstRectROI.height == oDstSize.height &&
...
#define DALI_CHECK_NPP(code)                                                           \
  do {                                                                                 \
    ...
    if (status != NPP_SUCCESS) {                                                       \
      ...
      dali::string error =                                                             \
          "[" + file + ":" + line + "]: NPP error \"" + nppErrorString(status) + "\""; \
      DALI_FAIL(error);                                                                \
    }                                                                                  \
  } while (0)
