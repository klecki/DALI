...
template <typename Out, typename In>
KernelRequirements NormalizeGPU<Out, In>::Setup(KernelContext &ctx,
                                                const TensorListShape<> &data_shape,
                                                const TensorListShape<> &param_shape,
                                                bool scalar_base, bool scalar_scale,
...
KernelRequirements NormalizeGPU<Out, In>::Setup(KernelContext &ctx,
                                                const TensorListShape<> &data_shape,
                                                span<const int> axes, bool scalar_base,
...
template <typename Out, typename In>
void NormalizeGPU<Out, In>::Run(KernelContext &ctx, const OutListGPU<Out> &out,
                                const InListGPU<In> &in, const InListGPU<Base> &base,
                                const InListGPU<Scale> &scale, float global_scale, float shift,
                                float epsilon) {
...
template <typename Out, typename In>
void NormalizeGPU<Out, In>::Run(KernelContext &ctx, const OutListGPU<Out> &out,
                                const InListGPU<In> &in, float base, const InListGPU<Scale> &scale,
...
template <typename Out, typename In>
void NormalizeGPU<Out, In>::Run(KernelContext &ctx, const OutListGPU<Out> &out,
                                const InListGPU<In> &in, const InListGPU<Base> &base, float scale,
                                float global_scale, float shift, float epsilon) {
  assert(impl_ && "Call setup first");
  impl_->Run(ctx, out, in, base, scale, global_scale, shift, epsilon);
}
...
template <typename Out, typename In>
void NormalizeGPU<Out, In>::Run(KernelContext &ctx, const OutListGPU<Out> &out,
                                const InListGPU<In> &in, float base, float scale,
                                float global_scale, float shift, float epsilon) {
  assert(impl_ && "Call setup first");
  impl_->Run(ctx, out, in, base, scale, global_scale, shift, epsilon);
}
