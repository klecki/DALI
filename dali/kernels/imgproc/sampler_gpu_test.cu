...
__global__ void RunSampler2D(Surface2D<Out> out, Sampler2D<interp, In> sampler, In border_value,
...
template <typename Out, DALIInterpType interp, int MaxChannels = 8, typename In>
__global__ void RunSampler3D(Surface3D<Out> out, Sampler3D<interp, In> sampler, In border_value,
  ...
  for (int oz = 0; oz < d; oz++) {
    ...
    for (int oy = 0; oy < h; oy++) {
      ...
      for (int ox = 0; ox < w; ox++) {
        ...
        Pixel ref;
        if (ix < 0 || iy < 0 || iz < 0 || ix >= surf_cpu.size.x || iy >= surf_cpu.size.y ||
            iz >= surf_cpu.size.z) {
          ref = border;
        } else {
          for (int c = 0; c < surf_cpu.channels; c++)
