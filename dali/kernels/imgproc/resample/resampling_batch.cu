...
__global__ void BatchedSeparableResampleKernel(
    int which_pass, const SampleDesc<spatial_ndim> *__restrict__ samples,
  ...
  switch (ftype) {
    case ResamplingFilterType::Nearest: {
      ...
      scale_v[axis] = scale;
      NNResample(lo, hi, origin_v, scale_v, sample_out, out_strides, sample_in, in_strides,
                 in_shape, sample.channels);
    } break;
    case ResamplingFilterType::Linear:
      if (axis == 0) {
        LinearHorz(lo, hi, origin, scale, sample_out, out_strides, sample_in, in_strides, in_shape,
                   sample.channels);
      } else if (axis == 1) {
        LinearVert(lo, hi, origin, scale, sample_out, out_strides, sample_in, in_strides, in_shape,
                   sample.channels);
      } else {
        LinearDepth(lo, hi, origin, scale, sample_out, out_strides, sample_in, in_strides, in_shape,
                    sample.channels);
      }
      break;
    default:
      if (axis == 0) {
        ResampleHorz(lo, hi, origin, scale, sample_out, out_strides, sample_in, in_strides,
                     in_shape, sample.channels, filter, support);
      } else if (axis == 1) {
        ResampleVert(lo, hi, origin, scale, sample_out, out_strides, sample_in, in_strides,
                     in_shape, sample.channels, filter, support);
      } else if (axis == 2) {
        ResampleDepth(lo, hi, origin, scale, sample_out, out_strides, sample_in, in_strides,
                      in_shape, sample.channels, filter, support);
      }
      break;
  }
...
#define INSTANTIATE_BATCHED_RESAMPLE(spatial_ndim, Output, Input)                    \
  template DLL_PUBLIC void BatchedSeparableResample<spatial_ndim, Output, Input>(    \
      int which_pass, const SampleDesc<spatial_ndim> *samples,                       \
      const BlockDesc<spatial_ndim> *block2sample, int num_blocks, ivec3 block_size, \
      cudaStream_t stream)
