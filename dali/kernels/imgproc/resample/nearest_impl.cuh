...
__device__ void NNResample(ivec2 lo, ivec2 hi, vec2 origin, vec2 scale, Dst *__restrict__ out,
                           vec<1, ptrdiff_t> out_stride, const Src *__restrict__ in,
                           vec<1, ptrdiff_t> in_stride, ivec2 in_size, int channels) {
  origin += 0.5f * scale;
 ...
 */
template <typename Dst, typename Src>
__device__ void NNResample(ivec3 lo, ivec3 hi, vec3 origin, vec3 scale, Dst *__restrict__ out,
                           vec<2, ptrdiff_t> out_stride, const Src *__restrict__ in,
                           vec<2, ptrdiff_t> in_stride, ivec3 in_size, int channels) {
  origin += 0.5f * scale;
