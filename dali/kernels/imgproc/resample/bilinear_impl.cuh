...
__device__ void LinearHorz_Channels(ivec2 lo, ivec2 hi, float src_x0, float scale,
...
__device__ void LinearHorz(ivec<spatial_ndim> lo, ivec<spatial_ndim> hi, float src_x0, float scale,
...
__device__ void LinearVert(ivec2 lo, ivec2 hi, float src_y0, float scale, Dst *__restrict__ out,
                           ptrdiff_vec<1> out_strides, const Src *__restrict__ in,
                           ptrdiff_vec<1> in_strides, ivec2 in_shape, int channels) {
...
template <typename Dst, typename Src>
__device__ void LinearDepth(ivec2 lo, ivec2 hi, float src_x0, float scale, Dst *__restrict__ out,
                            ptrdiff_vec<1> out_strides, const Src *__restrict__ in,
                            ptrdiff_vec<1> in_strides, ivec2 in_shape, int channels) {
...
__device__ void LinearHorz_Channels(ivec3 lo, ivec3 hi, float src_x0, float scale,
                                    Dst *__restrict__ out, ptrdiff_vec<2> out_strides,
                                    const Src *__restrict__ in, ptrdiff_vec<2> in_strides,
                                    ivec3 in_shape, int channels) {
  for (int z = lo.z + threadIdx.z; z < hi.z; z += blockDim.z) {
    ptrdiff_t out_ofs = z * out_strides.y;
    ptrdiff_t in_ofs = z * in_strides.y;
    LinearHorz_Channels<static_channels>(sub<2>(lo), sub<2>(hi), src_x0, scale, out + out_ofs,
                                         sub<1>(out_strides), in + in_ofs, sub<1>(in_strides),
                                         sub<2>(in_shape), channels);
  }
}

template <typename Dst, typename Src>
__device__ void LinearHorz(ivec3 lo, ivec3 hi, float src_x0, float scale, Dst *__restrict__ out,
                           ptrdiff_vec<2> out_strides, const Src *__restrict__ in,
                           ptrdiff_vec<2> in_strides, ivec3 in_shape, int channels) {
  VALUE_SWITCH(channels, static_channels, (1, 2, 3, 4),
  (
    LinearHorz_Channels<static_channels>(
      lo, hi, src_x0, scale,
...
template <typename Dst, typename Src>
__device__ void LinearVert(ivec3 lo, ivec3 hi, float src_y0, float scale, Dst *__restrict__ out,
                           ptrdiff_vec<2> out_strides, const Src *__restrict__ in,
                           ptrdiff_vec<2> in_strides, ivec3 in_shape, int channels) {
  for (int z = lo.z + threadIdx.z; z < hi.z; z += blockDim.z) {
    ptrdiff_t out_ofs = z * out_strides.y;
    ptrdiff_t in_ofs = z * in_strides.y;
    LinearVert(sub<2>(lo), sub<2>(hi), src_y0, scale, out + out_ofs, sub<1>(out_strides),
 ...
 */
template <typename Dst, typename Src>
__device__ void LinearDepth(ivec3 lo, ivec3 hi, float src_z0, float scale, Dst *__restrict__ out,
                            ptrdiff_vec<2> out_strides, const Src *__restrict__ in,
                            ptrdiff_vec<2> in_strides, ivec3 in_shape, int channels) {
