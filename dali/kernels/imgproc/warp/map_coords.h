...
DALI_HOST_DEV inline enable_if_t<is_fp_mapping<Mapping>::value, vec<dim>> map_coords(
    const Mapping &m, ivec<dim> pos) {
 ...
 */
template <typename Mapping, int dim>
DALI_HOST_DEV inline enable_if_t<!is_fp_mapping<Mapping>::value, ivec<dim>> map_coords(
    const Mapping &m, ivec<dim> pos) {
