...
template <typename Out, typename In, typename W, bool has_channels>
struct SeparableConvolution<Out, In, W, 2, has_channels> {
  static constexpr int ndim = has_channels ? 3 : 2;

  //todo?, array of windows?
  KernelRequirements Setup(KernelContext& ctx, const InTensorCPU<In, ndim>& in, const std::array<TensorView<StorageCPU, const W, 1>, 2>& windows) {
    KernelRequirements req;
  ...
  void Run(KernelContext& ctx, const TensorView<StorageCPU, Out, ndim> out,
           const TensorView<StorageCPU, const In, ndim>& in, const std::array<TensorView<StorageCPU, const W, 1>, 2>& windows) {
