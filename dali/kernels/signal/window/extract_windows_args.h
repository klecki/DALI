...
enum class Padding { None, Zero, Reflect };
 ...
 */
DALI_HOST_DEV DALI_FORCEINLINE constexpr int64_t num_windows(int64_t length, int window_size,
                                                             int step, bool centered) {
  if (!centered) length -= window_size;
...
struct ExtractWindowsArgs {
  ...
  constexpr inline bool operator==(const ExtractWindowsArgs& oth) const {
    return window_length == oth.window_length && window_center == oth.window_center &&
           window_step == oth.window_step && axis == oth.axis && padding == oth.padding;
  }

  DALI_HOST_DEV DALI_FORCEINLINE constexpr bool operator!=(const ExtractWindowsArgs& oth) const {
    return !operator==(oth);
  }
