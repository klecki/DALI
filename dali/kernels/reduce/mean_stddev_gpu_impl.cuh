...
template <typename Out, typename In, typename Actual, typename Postprocessor = ScaleAndConvert<Out>>
class MeanImplBase {
 public:
  Actual &This() {
  ...
  Postprocessor GetPostprocessorImpl(int sample_index, bool reduce_batch) const {
    int64_t reduced_elems =
        reduce_batch ? This().TotalReducedElements() : This().ReducedElements(sample_index);
...
template <typename Out, typename In,
          typename Acc = default_sum_acc_t<Out, decltype(reductions::square()(In()))>>
class MeanSquareImplGPU : public ReduceImplGPU<Out, In, Acc, MeanSquareImplGPU<Out, In, Acc>>,
 ...
 public:
  ...
  template <int non_reduced_dims>
  PreprocessorBank<non_reduced_dims> *GetPreprocessorBanksImpl(WorkArea &wa, int axis,
                                                               int_const<non_reduced_dims>) const {
    return nullptr;
  }
...
struct VariancePreprocessorBank<1, Mean> {
  ...
  DALI_HOST_DEV DALI_FORCEINLINE reductions::variance<Mean> Get(const i64vec<1> &pos) const {
...
struct VariancePreprocessorBank<2, Mean> {
  ...
  DALI_HOST_DEV DALI_FORCEINLINE reductions::variance<Mean> Get(const i64vec<2> &pos) const {
...
class VarianceImplBase {
 public:
  ...
  PreprocessorBank<1> *GetPreprocessorBanks(WorkArea &wa, int axis, int_const<1>) const {
  ...
  PreprocessorBank<2> *GetPreprocessorBanks(WorkArea &wa, int axis, int_const<2>) const {
  ...
  PreprocessorBank<non_reduced_dims> *GetPreprocessorBanksImpl(
      WorkArea &wa, int axis, int_const<non_reduced_dims> nrd) const {
...
class VarianceImplGPU : public ReduceImplGPU<Out, In, Acc, VarianceImplGPU<Out, In, Mean, Acc>>,
 ...
 public:
  ...
  typename MeanBase::postprocessor_t GetPostprocessorImpl(int sample_index,
                                                          bool reduce_batch) const {
    int64_t reduced_elems =
        reduce_batch ? this->TotalReducedElements() : this->ReducedElements(sample_index);
...
class StdDevImplGPU : public ReduceImplGPU<Out, In, Acc, StdDevImplGPU<Out, In, Mean, Acc>>,
 ...
 public:
  ...
  typename RMSBase::postprocessor_t GetPostprocessorImpl(int sample_index,
                                                         bool reduce_batch) const {
    int64_t reduced_elems =
        reduce_batch ? this->TotalReducedElements() : this->ReducedElements(sample_index);
...
class InvStdDevImplGPU
 ...
 public:
   ...
   */
  void Run(KernelContext &kctx, const OutListGPU<Out> &out, const InListGPU<In> &in,
           const InListGPU<Mean> &mean, int ddof = 0, float epsilon = 0.0f) {
    this->InitMean(mean);
