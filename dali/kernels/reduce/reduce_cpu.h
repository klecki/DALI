...
void reduce1D_stride(Dst &reduced, const Src *data, int64_t dynamic_stride, int64_t n,
  ...
  if (n > kTreeReduceThreshold) {
    ...
    Dst tmp = neutral;
    for (int64_t i = 0; i < n; i++) R(tmp, P(data[i * stride]));
...
struct ReduceBaseCPU {
  void Setup(const OutTensorCPU<Dst, -1> &out, const InTensorCPU<Src, -1> &in,
             span<const int> axes) {
    if (in.shape.size() > 64) throw std::range_error("Reduce supports up to 64 dimensions");
    input = in;
    output = out;
    Actual &self = This();
    self.InitAxes(axes);
  ...
  void Run(bool clear = true, bool postprocess = true) {
    ...
    if (postprocess) This().PostprocessAll();
  }
  ...
  void PostprocessAll() {
    ...
    for (int64_t i = 0; i < v; i++) output.data[i] = This().Postprocess(output.data[i]);
  }
 ...
 protected:
  ...
  void SetupOutput() {
    ...
    // full reduction?
    if (output.shape.empty()) output.shape = {1};
  }
...
struct MeanCPU : ReduceBaseCPU<Dst, Src, MeanCPU<Dst, Src>> {
  void PostSetup() {
    int64_t v = 1;
    for (auto a : this->axes) v *= this->input.shape[a];
    norm_factor = 1.0 / v;
  }
