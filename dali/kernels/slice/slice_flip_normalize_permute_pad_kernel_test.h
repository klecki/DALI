  ...
  void PrepareExpectedOutput(TestTensorList<InputType, Dims>& input_data,
    ...
    for (int i = 0; i < in.size(); i++) {
      ...
      if (mean.size() > 1 && fill_values.size() == 1) {
        for (size_t i = 1; i < mean.size(); i++) fill_values.push_back(fill_values[0]);
      }
      int per_ch_arg_size = std::max(mean.size(), fill_values.size());
      if (per_ch_arg_size > 1 && channel_dim == -1) channel_dim = Dims - 1;
      ...
      for (int64_t out_idx = 0; out_idx < total_size; out_idx++) {
        ...
        for (int d = 0; d < Dims; d++) {
          ...
          int in_i_d =
              flip[perm_d] ? anchor[perm_d] + slice_shape[perm_d] - 1 - i_d : anchor[perm_d] + i_d;
          out_of_bounds |= in_i_d < 0 || in_i_d >= in_shape[perm_d];
          if (!out_of_bounds) in_idx += in_i_d * in_strides[perm_d];
        }
...
struct SliceFlipNormPermArgsGen_SliceFlipNormalizePermute_PermuteHWC2CHW {
  SliceFlipNormalizePermutePadArgs<Dims> Get(const TensorShape<Dims>& input_shape) {
    ...
    for (int d = 0; d < Dims; d++) {
      ...
      args.shape[d] = d == 0 || d == 1 ? input_shape[d] / 2 : input_shape[d];
      args.flip[d] = d == 0 || d == 1;
...
using SLICE_FLIP_NORMALIZE_PERMUTE_TEST_TYPES = ::testing::Types<
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_CopyOnly<float, 3>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_SliceOnly<float, 3>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_SliceOnly_WithAnchor<float, 3>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipHW<float, 3>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipDim<float, 3, 0>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipDim<float, 3, 1>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipDim<float, 3, 2>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_NormalizeOnly<float, 3>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_NormalizeOnly_Scalar<float, 3>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_NormalizeAndFlipDim<float, 3, 0>>,
    SliceTestArgs<int, float, 2, 1, 10,
                  SliceFlipNormPermArgsGen_PermuteOnly_ReversedDims<float, 2>>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_PermuteOnly_ReversedDims<float, 3>>,
    SliceTestArgs<int, float, 2, 1, 1, SliceFlipNormPermArgsGen_PermuteOnly_ReversedDims<float, 2>,
                  10, 2>,
    SliceTestArgs<int, float, 2, 1, 1, SliceFlipNormPermArgsGen_PermuteOnly_ReversedDims<float, 2>,
                  2, 10>,
    ...
    SliceTestArgs<int, float, 3, 1, 2,
                  SliceFlipNormPermArgsGen_SliceFlipNormalizePermute_PermuteHWC2CHW<float, 3>, 2,
                  2>,
    SliceTestArgs<int, float, 2, 1, 2, SliceFlipNormPermArgsGen_SlicePadFlip<float, 2, false, true>,
                  2, 2>,
    SliceTestArgs<int, float, 2, 1, 2, SliceFlipNormPermArgsGen_SlicePadFlip<float, 2, true, false>,
                  2, 2>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_SlicePadFlip<float, 3, true, true>,
                  2, 2>,
    SliceTestArgs<int, float, 3, 1, 2,
                  SliceFlipNormPermArgsGen_SlicePadNormalizePermute_PermuteHWC2CHW<float, 3>, 2, 2>,
    SliceTestArgs<int, float, 3, 1, 2,
                  SliceFlipNormPermArgsGen_SlicePadNormalizePermute_PermuteHWC2CHW<float, 3, false>,
                  2, 2>,
    SliceTestArgs<int, float, 3, 1, 2,
                  SliceFlipNormPermArgsGen_SlicePadFlipNormalizePermute_PermuteHWC2CHW<float, 3>, 2,
                  2>,
    SliceTestArgs<
        int, float, 3, 1, 2,
        SliceFlipNormPermArgsGen_SlicePadFlipNormalizePermute_PermuteHWC2CHW<float, 3, false>, 2,
        2>,
    SliceTestArgs<int, float, 3, 1, 2, SliceFlipNormPermArgsGen_SlicePadPermuteAll<float, 3>, 2, 2>,
    ...
    SliceTestArgs<uint8_t, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipPad_GivenDim<float, 3, 0>,
                  2, 2>,
    SliceTestArgs<uint8_t, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipPad_GivenDim<float, 3, 1>,
                  2, 2>,
    SliceTestArgs<uint8_t, float, 3, 1, 2, SliceFlipNormPermArgsGen_FlipPad_GivenDim<float, 3, 2>,
                  2, 2>,
    ...
    SliceTestArgs<int, bool, 3, 1, 2, SliceFlipNormPermArgsGen_SliceOnly<bool, 3>>,
    SliceTestArgs<int, float, 3, 1, 10, ArgsGen_SingleValuePad_PermuteHWC2CHW<float, 3>, 10, 10>,
    SliceTestArgs<int, float, 3, 1, 10, ArgsGen_SliceNormalizeFlip_PadChannels<float, 3>, 10, 10>>;

using SLICE_FLIP_NORMALIZE_PERMUTE_TEST_TYPES_CPU_ONLY = ::testing::Types<
    SliceTestArgs<uint8_t, float16, 3, 1, 2, SliceFlipNormPermArgsGen_SliceOnly<float16, 3>>,
    SliceTestArgs<float16, uint8_t, 3, 1, 2, SliceFlipNormPermArgsGen_SliceOnly<uint8_t, 3>>>;
