...
class BlockSetup {
 public:
  ...
  BlockSetup() {
    ...
    for (int i = 0; i < ndim && i < 2; i++) block_size[i] = 256;
  ...
  TensorListShape<tensor_ndim> GetOutputShape(const TensorListShape<tensor_ndim> &in_shape,
    ...
    for (int i = 0; i < in_shape.num_samples(); i++) {
      ...
      for (int j = 0; j < tensor_ndim; j++) {
        out_tshape[j] = (j == channel_dim) ? in_shape.tensor_shape_span(i)[channel_dim]
                                           : output_sizes[i][in_d++];
      }
    }
    return shape;
  ...
  void ValidateOutputShape(const TensorListShape<tensor_ndim> &out_shape,
    ...
    for (int i = 0; i < in_shape.num_samples(); i++) {
      ...
      int in_d = 0;
      for (int j = 0; j < tensor_ndim; j++) {
        expected_shape[j] = (j == channel_dim) ? in_shape.tensor_shape_span(i)[channel_dim]
                                               : output_sizes[i][in_d++];
      }
  ...
  span<const BlockDesc> Blocks() const {
    return make_span(blocks_);
  }
 ...
 private:
  ...
  void UniformSizeSetup(const TensorListShape<tensor_ndim> &output_shape) {
    ...
    grid_dim_ = {div_ceil(uniform_output_size_.x, uniform_block_size_.x),
