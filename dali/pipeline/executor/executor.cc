...
void Executor<WorkspacePolicy, QueuePolicy>::RunCPUImpl() {
  ...
  if (device_id_ < 0) {
    ...
    DALI_ENFORCE(graph_->NumOp(OpType::GPU) == 0,
                 "Cannot run a pipeline with GPU ops in CPU-only mode. Please provide "
    ...
    for (int i = 0; i < graph_->NumOp(OpType::MIXED) && !exec_error_; ++i) {
      ...
      DALI_ENFORCE(op_node.spec.GetSchema().name() == "MakeContiguous",
                   "Cannot run a pipeline with Mixed ops in CPU-only mode. Please provide "
                   "valid device id or change the operators' device.");
      for (auto tensor_id : op_node.children_tensors) {
        const TensorNode &tensor_node = graph_->Tensor(tensor_id);
        DALI_ENFORCE(tensor_node.producer.storage_device == StorageDevice::CPU, "");
      }
    }
  }
