...
add_input(workspace_t<op_type> &ws, const tensor_data_store_queue_t &storage, int queue_idx = 0) {
...
void Executor::SetupInputOutput(workspace_t<op_type> &ws, const OpGraph &graph,
  ...
  for (const auto &arg_pair : node.spec.ArgumentInputs()) {
    ...
    auto &queue =
        get_queue<OpType::SUPPORT, StorageDevice::CPU>(tensor_to_store_queue_[tid]);
