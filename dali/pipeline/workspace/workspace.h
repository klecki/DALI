...
class WorkspaceBase : public ArgumentWorkspace {
 public:
  ...
  void SetInput(int idx, InputType<CPUBackend> input) {
    SetHelper<InputType, CPUBackend>(idx, input, &cpu_inputs_, &cpu_inputs_index_,
                                     &input_index_map_, &cpu_inputs_, &cpu_inputs_index_,
                                     &gpu_inputs_, &gpu_inputs_index_, StorageDevice::CPU);
  }
  ...
  void SetInput(int idx, InputType<GPUBackend> input) {
    SetHelper<InputType, GPUBackend>(idx, input, &gpu_inputs_, &gpu_inputs_index_,
                                     &input_index_map_, &cpu_inputs_, &cpu_inputs_index_,
                                     &gpu_inputs_, &gpu_inputs_index_, StorageDevice::GPU);
  }
  ...
  void SetOutput(int idx, OutputType<CPUBackend> output) {
    SetHelper<OutputType, CPUBackend>(idx, output, &cpu_outputs_, &cpu_outputs_index_,
                                      &output_index_map_, &cpu_outputs_, &cpu_outputs_index_,
                                      &gpu_outputs_, &gpu_outputs_index_, StorageDevice::CPU);
  }
  ...
  void SetOutput(int idx, OutputType<GPUBackend> output) {
    SetHelper<OutputType, GPUBackend>(idx, output, &gpu_outputs_, &gpu_outputs_index_,
                                      &output_index_map_, &cpu_outputs_, &cpu_outputs_index_,
                                      &gpu_outputs_, &gpu_outputs_index_, StorageDevice::GPU);
  }
 ...
 protected:
  ...
  template <typename T>
  void AddHelper(T entry, vector<T>* vec, vector<int>* index, vector<InOutMeta>* index_map,
                 StorageDevice storage_device) {
  ...
  void SetHelper(int idx, T<Backend> entry, vector<T<Backend>>* vec, vector<int>* index,
                 vector<InOutMeta>* index_map, vector<T<CPUBackend>>* cpu_vec,
                 vector<int>* cpu_index, vector<T<GPUBackend>>* gpu_vec, vector<int>* gpu_index,
                 StorageDevice storage_device) {
  ...
  inline const InputType<GPUBackend>& GPUInput(int idx) const {
    ...
    DALI_ENFORCE(tensor_meta.storage_device == StorageDevice::GPU,
                 "Input with given "
                 "index (" +
                     std::to_string(idx) + ") does not have the calling backend type (GPUBackend)");
  ...
  inline const InputType<CPUBackend>& CPUInput(int idx) const {
    ...
    DALI_ENFORCE(tensor_meta.storage_device == StorageDevice::CPU,
                 "Input with given "
                 "index (" +
                     std::to_string(idx) + ") does not have the calling backend type (CPUBackend)");
 ...
 private:
  inline const InOutMeta& FetchAtIndex(const vector<InOutMeta>& index_map, int idx) const {
    DALI_ENFORCE(idx >= 0 && idx < (int)index_map.size(),
                 "Index out of range." + std::to_string(idx) + " not in range [0, " +
