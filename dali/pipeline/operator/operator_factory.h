...
class OperatorRegistry {
 public:
  ...
  void Register(const std::string &name, Creator creator, const std::string &devName = "") {
    ...
    DALI_ENFORCE(registry_.count(name) == 0, "Operator \"" + name + "\" already registered" +
                                                 (devName != "" ? (" for " + devName) : "") + ".");
    registry_[name] = creator;
  }

  std::unique_ptr<OpType> Create(const std::string &name, const OpSpec &spec,
                                 const std::string *devName = NULL) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto creator_it = registry_.find(name);
    DALI_ENFORCE(creator_it != registry_.end(), "Operator \"" + name + "\" not registered" +
                                                    (devName ? (" for " + *devName) : "") + ".");
...
class Registerer {
 public:
  Registerer(const std::string &name, OperatorRegistry<OpType> *registry,
...
#define DALI_DEFINE_OPTYPE_REGISTERER(OpName, DerivedType, RegistryName, OpType, dev) \
  namespace {                                                                         \
