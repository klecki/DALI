...
std::unique_ptr<ExprImplBase> ExprImplFactory(const DeviceWorkspace &ws,
  ...
  switch (expr.GetSubexpressionCount()) {
    case 1:
      return ExprImplFactory1<ExprImplGpuT, GPUBackend>(ws, dynamic_cast<const ExprFunc&>(expr));
    case 2:
      return ExprImplFactory2<ExprImplGpuTT, ExprImplGpuTC, ExprImplGpuCT, GPUBackend>(ws, dynamic_cast<const ExprFunc&>(expr));
    default:
