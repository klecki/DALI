...
template <typename T>
inline std::enable_if_t<std::is_trivially_copyable<T>::value> CopyFunc(void *dst, const void *src,
                                                                       Index n) {
...
class DLL_PUBLIC TypeInfo {
 public:
   ...
   * @param use_copy_kernel If true, a copy kernel will be used instead of cudaMemcpyAsync when
   * applicable (only relevant for device and host pinned memory)
   */
...
#define DALI_REGISTER_TYPE(Type, dtype)                                                     \
  template <>                                                                               \
  DLL_PUBLIC string TypeTable::GetTypeName<Type>() DALI_TYPENAME_REGISTERER(Type, dtype);   \
  template <>                                                                               \
  DLL_PUBLIC DALIDataType TypeTable::GetTypeID<Type>() DALI_TYPEID_REGISTERER(Type, dtype); \
