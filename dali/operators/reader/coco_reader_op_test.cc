  ...
  std::vector<int> CopyIds(DeviceWorkspace &ws, int ids_out_idx = 3) {
    ...
    for (int sample_idx = 0; sample_idx < shape.num_samples(); sample_idx++) {
      MemCopy(
        ...
        shape[sample_idx].size() * sizeof(int));
      out_ptr += shape[sample_idx].size();
    }
  ...
  void CheckInstances(DeviceWorkspace &ws, bool ltrb, bool ratio, bool skip_empty,
    ...
    for (int sample_idx = 0; sample_idx < boxes_shape.num_samples(); sample_idx++) {
      MemCopy(
        ...
        boxes_shape[sample_idx].size() * sizeof(float));
      boxes_ptr += boxes_shape[sample_idx].size();
    }
    cudaStreamSynchronize(0);
    ...
    for (int sample_idx = 0; sample_idx < labels_shape.num_samples(); sample_idx++) {
      MemCopy(
        ...
        labels_shape[sample_idx].size() * sizeof(int));
      labels_ptr += labels_shape[sample_idx].size();
    }
    cudaStreamSynchronize(0);
    ...
    if (polygon_masks || polygon_masks_legacy) {
      ...
      for (int sample_idx = 0; sample_idx < polygons_shape.num_samples(); sample_idx++) {
        MemCopy(
          ...
          polygons_shape[sample_idx].size() * sizeof(int));
        polygons_ptr += polygons_shape[sample_idx].size();
      }
      ...
      for (int sample_idx = 0; sample_idx < vertices_shape.num_samples(); sample_idx++) {
        MemCopy(
          ...
          vertices_shape[sample_idx].size() * sizeof(float));
        vertices_ptr += vertices_shape[sample_idx].size();
      }
      cudaStreamSynchronize(0);
