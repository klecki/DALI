...
class NumpyReader : public DataReader<CPUBackend, ImageFileWrapper> {
 public:
  explicit NumpyReader(const OpSpec& spec) : DataReader<CPUBackend, ImageFileWrapper>(spec) {
  ...
  void RunImpl(SampleWorkspace& ws) override {
    ...
    if (imfile.meta == "transpose:false") {
      ...
      std::memcpy(image_output.raw_mutable_data(), imfile.image.raw_data(), image_bytes);
    } else {
