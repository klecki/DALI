...
#define LUT_OUT_TYPES                                                                        \
  (uint8_t, uint16_t, uint32_t, uint64_t, int8_t, int16_t, int32_t, int64_t, float16, float, \
   double, bool)
...
class LookupTable : public Operator<Backend> {
 public:
  ...
  explicit inline LookupTable(const OpSpec &spec)
    ...
    DALI_ENFORCE(keys.size() == values_f.size(), "`keys` size should match `values` size");
    ...
    TYPE_SWITCH(output_type_, dali::type2id, OutputType, LUT_OUT_TYPES, (
        ...
        for (size_t i = 0; i < kLookupTableSize; i++) { values[i] = ConvertSat<OutputType>(default_value_f_);
        }
        auto keys_size = keys.size();
        for (size_t i = 0; i < keys_size; i++) { values[keys[i]] = ConvertSat<OutputType>(values_f[i]);
        }
