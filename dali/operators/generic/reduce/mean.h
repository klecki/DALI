...
#define MEAN_TYPES_MAP                                                                            \
  (((uint8_t), (uint8_t, float)), ((int8_t), (int8_t, float)), ((uint16_t), (uint16_t, float)),   \
   ((int16_t), (int16_t, float)), ((uint32_t), (uint32_t, float)), ((int32_t), (int32_t, float)), \
...
class MeanOp : public Reduce<ReductionType, Backend, MeanOp> {
 public:
  explicit inline MeanOp(const OpSpec &spec) : Reduce<ReductionType, Backend, MeanOp>(spec) {}
  ...
  void RunImplImpl(workspace_t<Backend> &ws) {
    ...
    TYPE_MAP(input_type, output_type, type2id, InputType, OutputType, MEAN_TYPES_MAP,
