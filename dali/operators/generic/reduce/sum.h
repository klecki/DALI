...
#define SUM_TYPES_MAP                                                                 \
  (((uint8_t), (uint8_t, uint64_t, float)), ((int8_t), (int8_t, int64_t, float)),     \
   ((uint16_t), (uint16_t, uint64_t, float)), ((int16_t), (int16_t, int64_t, float)), \
   ((uint32_t), (uint32_t, uint64_t, float)), ((int32_t), (int32_t, int64_t, float)), \
   ((uint64_t), (uint64_t)), ((int64_t), (int64_t)), ((float), (float)))
...
class SumOp : public Reduce<ReductionType, Backend, SumOp> {
 public:
  explicit inline SumOp(const OpSpec &spec) : Reduce<ReductionType, Backend, SumOp>(spec) {}
  ...
  void RunImplImpl(workspace_t<Backend> &ws) {
    ...
    TYPE_MAP(input_type, output_type, type2id, InputType, OutputType, SUM_TYPES_MAP,
