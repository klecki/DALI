...
std::vector<kernels::EraseArgs<T, Dims>> GetEraseArgs(const OpSpec &spec,
  ...
  if (!has_tensor_roi_anchor) roi_anchor = spec.template GetArgument<std::vector<float>>("anchor");
  ...
  if (!has_tensor_roi_shape) roi_shape = spec.template GetArgument<std::vector<float>>("shape");
  ...
  DALI_ENFORCE(
      channels_dim >= 0 || fill_value.size() <= 1,
