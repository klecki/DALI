 ...
 *                           (the audio data is expected to be resampled if its original sampling
 * rate differs)
 * @param downmix If set to true, the audio channels are expected to be downmixed, resulting in a
 * shape with 1 dimension ({nsamples,}), instead of 2 ({nsamples, nchannels})
 */
 ...
 * @param audio Destination buffer. The function will decode as many audio samples as the shape of
 * this argument
 * @param decoder Decoder object.
 * @param meta Audio metadata.
 * @param resampler Resampler instance used if resampling is required
 * @param decode_scratch_mem Scratch memory used for decoding, when decoding can't be done directly
 * to the output buffer. If downmixing or resampling is required, this buffer should have a positive
 * length, representing decoded audio length at the original sampling rate: ``length * nchannels``
 * @param resample_scratch_mem Scratch memory used for the input of resampling.
 *                             If resampling is required, the buffer should have a positive length,
 * representing the decoded audio length, ``length`` if downmixing is enabled, or the decoded audio
 * length including channels, ``length * nchannels``, otherwise.
 * @param target_sample_rate If a positive value is provided, the signal will be resampled except
 * when its original sampling rate is equal to the target.
