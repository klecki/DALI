...
class CropMirrorNormalize : public Operator<Backend> {
 public:
  explicit inline CropMirrorNormalize(const OpSpec &spec)
    ...
    if (mean_vec_.size() != inv_std_vec_.size()) {
      if (mean_vec_.size() == 1) mean_vec_.resize(args_size, mean_vec_[0]);

      if (inv_std_vec_.size() == 1) inv_std_vec_.resize(args_size, inv_std_vec_[0]);
    }
 ...
 protected:
  ...
  void SetupCommonImpl(const workspace_t<Backend> &ws) {
    ...
    if (output_type_ == DALI_NO_TYPE) output_type_ = input_type_;
