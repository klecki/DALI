...
class CropAttr {
 public:
  ...
  explicit inline CropAttr(const OpSpec& spec)
      : spec__(spec), batch_size__(spec__.GetArgument<int>("batch_size")) {
    ...
    if (has_crop_d_) crop_depth_.resize(batch_size__, crop_d);
    crop_x_norm_.resize(batch_size__, 0.0f);
    crop_y_norm_.resize(batch_size__, 0.0f);
    if (has_crop_d_) crop_z_norm_.resize(batch_size__, 0.0f);
  ...
  void ProcessArguments(const ArgumentWorkspace* ws, std::size_t data_idx) {
    ...
    if (has_crop_d_) crop_z_norm_[data_idx] = spec__.GetArgument<float>("crop_pos_z", ws, data_idx);
    if (spec__.ArgumentDefined("crop_w")) {
      crop_width_[data_idx] = static_cast<int>(spec__.GetArgument<float>("crop_w", ws, data_idx));
    }
    if (spec__.ArgumentDefined("crop_h")) {
      crop_height_[data_idx] = static_cast<int>(spec__.GetArgument<float>("crop_h", ws, data_idx));
    }
    if (spec__.ArgumentDefined("crop_d")) {
      crop_depth_[data_idx] = static_cast<int>(spec__.GetArgument<float>("crop_d", ws, data_idx));
    }
