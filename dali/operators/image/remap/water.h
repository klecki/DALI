...
class WaterAugment {
 public:
  ...
  explicit WaterAugment(const OpSpec &spec) : x_desc_(spec, "_x"), y_desc_(spec, "_y") {}
  ...
  vec2 operator()(float h, float w, int c, int H, int W, int C) {
    ...
    return {w + wX.ampl * sinf(wX.freq * h + wX.phase), h + wY.ampl * cosf(wY.freq * w + wY.phase)};
  }
...
class Water : public DisplacementFilter<Backend, WaterAugment> {
 public:
  explicit Water(const OpSpec &spec) : DisplacementFilter<Backend, WaterAugment>(spec) {}
