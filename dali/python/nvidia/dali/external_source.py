...
class _ExternalSourceGroup(object):
    ...
    def prefetch(self, pool, context_i, batch_size, epoch_idx):
        ...
        while (
            context.scheduled_ahead < self.prefetch_queue_depth and
        ...
        **kwargs
        ...
        if parallel:
            ...
            if prefetch_queue_depth < 1:
                raise ValueError(
                    "``prefetch_queue_depth`` must be a positive integer, got {}.".
                    format(prefetch_queue_depth)
                )
            ...
            for i in range(self._num_outputs):
                if dtype is not None:
                    if isinstance(dtype, (list, tuple)):
                        kwargs['dtype'] = dtype[
                            i] if i < len(dtype) else nvidia.dali.types.DALIDataType.NO_TYPE
                    else:
                        kwargs['dtype'] = dtype
                if ndim is not None:
