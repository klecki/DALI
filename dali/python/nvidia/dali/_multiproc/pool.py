...
class CallbackContext:
    ...
    def __init__(
        self, source_desc: SourceDescription, shm_manager: ShmChunkManager,
...
class WorkerContext:
    def __init__(
        self, source_descs: SourceDescription, dedicated_task_queue: Optional[ShmQueue],
...
def create_worker_contexts(
    ...
    general_cb_contexts = [
        i
        for i, cb_context in enumerate(callback_contexts) if cb_context.dedicated_worker_id is None
    ]
    worker_contexts = []
...
class ProcPool:
    ...
    """
    def __init__(
        self, mp, workers_contexts: List[WorkerContext], result_queue: ShmQueue,
...
def create_shm_chunk_manager_for_group(
    ...
    return ShmChunkManager(
        shm_pool, keep_alive_queue_size + group.prefetch_queue_depth, initial_chunk_size,
        num_mini_batches
    )
    ...
    def from_groups(
        ...
        shm_managers = [
            create_shm_chunk_manager_for_group(
                group, shm_pool, keep_alive_queue_size, min_initial_chunk_size, num_workers,
                batch_size
            ) for group in groups
        ]
        contexts = [
