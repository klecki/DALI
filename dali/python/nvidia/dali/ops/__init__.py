...
class _OperatorInstance(object):
    ...
    def generate_outputs(self):
        ...
        num_output = (
            self._op.schema.CalculateOutputs(self._spec) +
...
def _load_readers_tfrecord():
    ...
    for op_reg_name, op_class in [
        ('readers__TFRecord', tfrecord.TFRecord), ('TFRecordReader', tfrecord.TFRecordReader)
    ]:
...
def _preprocess_inputs(inputs, op_name, device, schema=None):
    ...
    def is_input(x):
        ...
        return (
            isinstance(x, (list)) and any(isinstance(y, _DataNode) for y in x) and
