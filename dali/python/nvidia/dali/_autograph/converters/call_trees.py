...
class _ArgTemplateBuilder(object):
  ...
  def add_stararg(self, a):
    ...
    self._argspec.append(
        gast.Call(gast.Name('tuple', ctx=gast.Load(), annotation=None, type_comment=None),
                  args=[a],
                  keywords=()))
...
class CallTreeTransformer(converter.Base):
  ...
  def _kwargs_to_dict(self, node):
    ...
    if node.keywords:
      return gast.Call(gast.Name('dict', ctx=gast.Load(), annotation=None, type_comment=None),
                       args=(),
  ...
  def visit_Call(self, node):
    ...
    if full_name in ('pdb.set_trace', 'ipdb.set_trace', 'breakpoint'):
      ...
      if not set_trace_warned:
        ...
        ag_logging.warning('Detected `pdb.set_trace()` in user code. The code'
    ...
    """
    new_call = templates.replace_as_expression(template,
                                               func=node.func,
