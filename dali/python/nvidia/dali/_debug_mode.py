...
from nvidia.dali._utils.external_source_impl import (
    get_callback_from_source as _get_callback_from_source, accepted_arg_count as _accepted_arg_count
)
...
class _OperatorManager:
    ...
    """
    def __init__(
        self, op_class, op_name, pipe, source_context, next_logical_id, batch_size, device_id, seed,
        inputs, kwargs
    ):
    ...
    def _pack_to_data_node_debug(self, data, position=None):
        ...
        return DataNodeDebug(
            data, self._op_name + position_to_suffix(position),
    ...
    def _prep_input_sets(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            if (
                not isinstance(input, (_tensors.TensorListCPU, _tensors.TensorListGPU)) and not (
                    isinstance(input, list) and all(
                        [
