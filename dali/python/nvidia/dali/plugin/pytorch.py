...
class DALIClassificationIterator(DALIGenericIterator):
    ...
    def __init__(self, pipelines, size=-1, reader_name=None, auto_reset=False, fill_last_batch=None,
        ...
        super(DALIClassificationIterator,
              self).__init__(pipelines, ["data", "label"], size, reader_name=reader_name,
                             auto_reset=auto_reset, fill_last_batch=fill_last_batch,
                             dynamic_shape=dynamic_shape, last_batch_padded=last_batch_padded,
...
class DALIRaggedIterator(_DaliBaseIterator):
    ...
    def __init__(self, pipelines, output_map, size=-1, reader_name=None, output_types=None,
        ...
        # check the assert first as _DaliBaseIterator would run the prefetch
        self._output_tags = {
            DALIRaggedIterator.DENSE_TAG, DALIRaggedIterator.SPARSE_LIST_TAG,
        ...
        super(DALIRaggedIterator,
              self).__init__(pipelines, size, reader_name, auto_reset, fill_last_batch,
                             last_batch_padded, last_batch_policy, prepare_first_batch)
                ...
                if self._outputs_types is None or \
                   self._outputs_types[j] == DALIRaggedIterator.DENSE_TAG:
                    ...
                    pyt_tensors[category] = [
                        torch.empty(shape, dtype=category_torch_type[category],
                                    device=category_device[category])
                ...
                if self._outputs_types is None or \
                   self._outputs_types[j] == DALIRaggedIterator.DENSE_TAG:
                    ...
                    if self._outputs_types[j] == DALIRaggedIterator.SPARSE_COO_TAG:
                        ...
                        indices = [[(i, j) for j in range(shape[0])]
