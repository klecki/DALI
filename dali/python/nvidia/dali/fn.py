...
def _wrap_op_fn(op_class, wrapper_name, wrapper_doc):
    ...
    def fn_wrapper(*inputs, **kwargs):
        ...
        if wrapper_name not in {"merge"}:
            # Inputs to merge are generated by the AutoGraph transformation, the predicate
            # might need the split though
            inputs_bkp = list(inputs)
            for i, input in enumerate(inputs):
                if isinstance(input, DataNode):
                    inputs_bkp[i] = _conditionals._CONDITION_STACK.preprocess_input(input)
            inputs = tuple(inputs_bkp)
            for key, arg in kwargs.items():
                if isinstance(arg, DataNode):
                    kwargs[key] = _conditionals._CONDITION_STACK.preprocess_input(arg)
